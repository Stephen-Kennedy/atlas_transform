...
def render_atlas_block(
    ...
    if not quick_wins_blocks:
        lines.append("#### ⚡ Quick Wins (manual pick: 1 unit)")
        lines.append(ph())
    else:
...
def build_fill_request(atlas_block: str, atlas_date: Optional[str] = None) -> Dict:
    ...
    block_header_re = re.compile(r"^(?:-\s+|#{4}\s+)(?:\d{4}|\d{2}:\d{2})\s*[-–]\s*(?:\d{4}|\d{2}:\d{2}):?\s+(.*)$")
...
def apply_fill_plan(
    ...
    for ln in atlas_block.splitlines():
        ...
        if (ln.lstrip().startswith("- ") or ln.lstrip().startswith("####")) and re.search(r"\bunit[s]?\b", s, re.IGNORECASE):
            current_kind = "QUICK_WINS"
            out_lines.append(ln)
            continue
